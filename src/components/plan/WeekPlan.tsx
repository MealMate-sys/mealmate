...
import {
  getWeekStart, getPrevWeek, getNextWeek,
  addDays, formatDate, isCurrentWeek,
} from '@/lib/week'
...
export function WeekPlan({
  ...
  const [mobileAddSlot, setMobileAddSlot] = useState<{ dayIndex: number; slot: Slot } | null>(null)
    ...
    >
      <div className="flex flex-col gap-6">
        ...
        <div className="flex gap-6 items-start">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {SLOTS.map(({ key, label }) => (
                    <DroppableSlot
                      ...
                      onMobileAdd={(dayIndex, slot) => {
                        setMobileAddSlot({ dayIndex, slot })
                        setMobileSearch('')
                      }}
                    />
                  ))}
          ...
          {/* Sidebar — nur Desktop */}
          <div className="hidden sm:flex w-64 shrink-0 flex-col gap-3 sticky top-24 bg-white rounded-2xl border border-cream-200 shadow-card p-4">
            <div className="flex items-center justify-between">
        ...
        <div className="flex flex-col gap-4">
          <div className="relative">
            ...
            <input
              ...
              className="w-full rounded-xl border border-cream-300 bg-cream-50 pl-8 pr-3 py-2.5 text-sm text-warm-900 placeholder:text-warm-700/40 focus:border-sage-500 focus:outline-none focus:ring-2 focus:ring-sage-500/20 transition"
              autoFocus
            />
          </div>
          ...
          <div className="flex flex-col gap-2 max-h-[50vh] overflow-y-auto">
            {recipes
              .filter((r) => {
                const matchesSearch = r.title.toLowerCase().includes(mobileSearch.toLowerCase())
                const matchesTag = activeTag ? r.tags?.includes(activeTag) : true
                return matchesSearch && matchesTag
              })
              .map((recipe) => (
                <button
                  key={recipe.id}
                  onClick={async () => {
                    if (!mobileAddSlot) return
                    await assignRecipeToSlot(recipe, mobileAddSlot.dayIndex, mobileAddSlot.slot)
                    setMobileAddSlot(null)
                    setMobileSearch('')
                  }}
                  className="flex items-center gap-3 rounded-xl border border-cream-200 bg-white px-3 py-3 text-left hover:border-sage-400 hover:bg-sage-50 transition active:scale-95"
                >
                  <span className="text-2xl">{recipe.cover_emoji ?? '🍽️'}</span>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-warm-900 truncate">{recipe.title}</p>
                    {recipe.tags && recipe.tags.length > 0 && (
                      <p className="text-xs text-warm-700/50 mt-0.5">
